...
@section('content')
    <main>
        <div class="article__main">
            <h1 class="article__main__title" style="text-align: center;">{{$article->title}}</h1>
            <div class="article__info">
                ...
                <span>
                    ...
                    {{ formatDate($article->publication) }}
                    {{-- 27 мая 2022 --}}
                </span>
            </div>
            <img class="article__cover" src="/{{$article->img}}" alt="Обложка">
            <p class="article__main__content">{!! nl2br($article->content) !!}</p>
        </div>

        <div class="pagination" style="margin-top: 20px">
                <a @if($nextPost) href="/article/{{ $nextPost->url }}  " @endif class="pagination__item">
                    @if($nextPost)
                ...
                <a @if($previousPost) href="/article/{{ $previousPost->url }}"  @endif class="pagination__item">
                    @if($previousPost)
